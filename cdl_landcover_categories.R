...
cdl <- read.csv2("https://raw.githubusercontent.com/ReproducibleQM/sapsuckRz/master/Data/cdl_landcover_data.csv", sep=",", check.names = F, na.strings = "0.0")
landtypes <- colnames(cdl)
landcodes <- updateNamesCDL(landtypes)

codekey <- data.frame(landtypes,landcodes)
view(codekey) #This dataframe lists all of the lables for the various codes we have. All of the codes presented have a "non-zero" presence near a site for at least one year, so all codes must be accounted for.
...
# Now let's build some categories for the landcover types. We don't need to distinguish between sorghum and barley or between watermelons and tomatoes. Some of these categories are simply sums of the smaller categories. Also, note that for now double crops are simply measured twice, once for each category they are in.
for(j in 1:length(cdl)){
  ...
  cdl$ag_corn[j] <- sum(cdl[j,c('1', '13', '225', '241', '12', '226', '237')])
  ...
  cdl$ag_other[j] <- sum(cdl[j,c('4', '61', '27', '36', '28', '59', '29', '44', '21', '6', '37', '32', '42', '41', '236', '68', '43','53', '47', '25', '74', '240', '254', '48', '67', '2', '209', '239', '46', '238', '70', '229', '216', '57', '10', '226', '76', '222', '235', '237', '224', '242', '31', '33', '11', '71', '54', '69', '246', '219', '66', '243', '250', '49', '50', '206', '221', '38', '244', '207', '77', '30', '14', '249', '245', '247', '220', '214', '55', '248', '35', '210', '34')])
...
cdl_sort <- cdl[,c('ID','State','Year','for_con','for_dec','for_mix','forest','ag_corn','ag_beans','ag_wheat','ag_other','ag','wet','urb','other')]
